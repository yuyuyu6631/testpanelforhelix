...
import { Card, Button, Input, Typography, Tabs, Alert, Tag } from 'antd';
...
export const PreviewPanel: React.FC<Props> = ({ template }) => {
    ...
    return (
        <Card title="Debug & Preview" size="small" style={{ height: '100%' }} styles={{ body: { height: 'calc(100% - 38px)', overflow: 'auto' } }}>
            ...
            {result && result.request && (
                <Tabs defaultActiveKey="1" items={[
                        ...
                        children: (
                            <div style={{ background: '#f5f5f5', padding: 8, borderRadius: 4 }}>
                                ...
                                <div style={{ marginTop: 8 }}>
                                    ...
                                    <pre style={{ fontSize: 11, background: '#eee', padding: 4 }}>{JSON.stringify(result.request.headers, null, 2)}</pre>
                                </div>
                                {result.request.body && (
                                    <div style={{ marginTop: 8 }}>
                                        <Text type="secondary">Body:</Text>
                                        <pre style={{ fontSize: 11, background: '#eee', padding: 4 }}>
                    ...
                    },
                    {
                        ...
                        children: result.response ? (
                            <div style={{ background: '#f5f5f5', padding: 8, borderRadius: 4 }}>
                                ...
                                {result.response.json ? (
                                    <pre style={{ fontSize: 11, overflow: 'auto', maxHeight: 400, background: '#eee', padding: 4 }}>
                        ...
                        ) : <Alert type="warning" message="No response data received" />
                    }
                ]} />
            )}
        </Card>

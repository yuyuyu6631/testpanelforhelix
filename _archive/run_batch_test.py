...
def run_batch_test():
    ...
    # 检查必要列 (支持中文或英文)
    if '问题' not in df.columns and 'query' not in df.columns:
        logger.error(f"测试文件缺少必要列 '问题' 或 'query'。当前列: {df.columns.tolist()}")
        return
        
    # 统一列名映射
    col_map = {
        'question': '问题' if '问题' in df.columns else 'query',
        'keywords': '预期关键字' if '预期关键字' in df.columns else 'keywords',
        'conditions': '预期条件' if '预期条件' in df.columns else 'conditions'
    }
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=max_workers) as executor:
        ...
        for index, row in df.iterrows():
            question = str(row.get(col_map['question'], ''))
        ...
        for future in concurrent.futures.as_completed(future_to_index):
            ...
            try:
                ...
                # 获取预期值
                exp_kw = row.get(col_map['keywords'], '')
                exp_cond = row.get(col_map['conditions'], '')
